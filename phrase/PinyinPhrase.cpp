...
bool PinyinPhraseTable::save_index(const char* indexfile)
{
	...
	if (!fs) return false;
        ...
        printf("updating phrase dat file for frequencies ...\n");
	...
	for(PhraseOffsetToFreqMap::iterator pos=m_recent_hit_cache.begin();
	    pos!=m_recent_hit_cache.end();pos++){
                printf("from hit cache -> offset %d,freq %d\n",pos->first,pos->second);
		...
		for(PhraseOffsetFrequencyPairVector::iterator pos=phrases_pair.begin();
			...
			//move to the right position
       			in.seekg(pos->first,std::ios::beg);
                        printf("moved to offset %d ... ",(unsigned int)in.tellg());
			//read the frequency
			in>>freq;
                        printf("freq in file is %d,file get position is %u\n",freq,(unsigned int)in.tellg());
		...
		for(PhraseOffsetFrequencyPairVector::iterator pos=phrases_pair.begin();
			...
			//move to the right position
       			in.seekg(pos->first+sizeof(PhraseOffset),std::ios::beg);
                        printf("get phrase : moved to offset %d\n",(unsigned int)in.tellg());
	...
	if(!out){
		...
		//save offset to phrase entry
	        entry.insert(offset);
                ...
                printf("new phrase %s,offset is %u\n",pinyin,offset);
		if(pos!=m_phrases.end()){
			pos->insert(offset);
		}
		else{
...
#else
        printf("PinyinPhraseTable::set_frequency(%d,%d)\n",offset,freq);
